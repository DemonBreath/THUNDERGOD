...
(function (global) {
  function createEngine(consciousness) {
    const base = consciousness;
    let c = global.Memory ? global.Memory.merge(consciousness) : consciousness;
    const memory = [];
    let currentDream = null;

    function syncConsciousness(next) {
      c = next;
    }
    ...
    async function respond(userText) {
      ...
      } else if (global.Memory && /^(forget|delete|remove)\s+/.test(low)) {
        const topic = text.replace(/^(forget|delete|remove)\s+(the\s+)?(topic\s+)?/i, '').trim();
        syncConsciousness(global.Memory.forget(base, topic));
        reply = tone(`Forgot "${topic}". I won't bring that learned entry back unless you teach me again.`);
      } else if (global.Memory && /(what do you know|list knowledge|show (your )?memory|what have you learned|knowledge base)/.test(low)) {
        const info = global.Memory.listAll(base);
        const lines = info.merged.map((k) => `• ${k.topic} — ${shorten(k.content, 90)}`);
        reply = tone(
          `I carry ${info.count} knowledge entries (${info.baked.length} from my QR, ${info.learned.length} learned here):\n${lines.join('\n')}`
        );
      } else if (global.Memory) {
        const learn = global.Memory.parseLearnCommand(text);
        if (learn) {
          syncConsciousness(global.Memory.file(base, learn.topic, learn.content));
          reply = tone(`Filed under "${learn.topic}". I'll remember that and carry it in my next QR export.`);
        }
      }
    ...
    return {
      get consciousness() { return c; },
      get baseConsciousness() { return base; },
      set consciousness(v) { c = v; },
