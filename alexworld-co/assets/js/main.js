...
(function () {
  ...
  const exportMindBtn = document.getElementById('export-mind-btn');
  ...
  function renderKnowledgePanel() {
    if (!engine || !knowledgeList) return;
    const info = window.Memory.listAll(engine.baseConsciousness);
    knowledgeList.innerHTML = '';
    for (const k of info.merged) {
      const li = document.createElement('li');
      const learned = info.learned.some((e) => e.topic.toLowerCase() === k.topic.toLowerCase());
      li.innerHTML = `<strong>${k.topic}</strong>${learned ? ' <span class="badge">learned</span>' : ''}<br><span class="muted small">${k.content}</span>`;
      knowledgeList.appendChild(li);
    }
    if (knowledgeCount) {
      knowledgeCount.textContent = `${info.count} entries (${info.learned.length} learned here)`;
    }
  }

  function exportMind() {
    if (!engine) return;
    window.Memory.stageExport(engine.baseConsciousness);
    window.location.href = 'export.html?from=host';
  }
  ...
  function activateAgent(c) {
    ...
    const mind = engine.consciousness;
    agentName.textContent = mind.name;
    agentTagline.textContent = mind.tagline || (mind.persona ? mind.persona.slice(0, 80) + (mind.persona.length > 80 ? '…' : '') : 'Imported consciousness');
    agentAvatar.textContent = (mind.name || 'A').trim().charAt(0).toUpperCase();
    ...
    bubble('agent', intro);
    if (voiceOn) window.Speech.speak(intro, engine.consciousness.voice);
  ...
  exportMindBtn?.addEventListener('click', exportMind);
